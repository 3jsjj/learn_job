...
% ------------------------- Model dimensions ----------------------------
% Actual uploaded Abaqus MTX uses six local DOFs per participating node:
%   1=U1, 2=U2, 3=U3, 4=UR1, 5=UR2, 6=UR3.
% The uploaded matrix contains DOFs 1...6 for every stiffness-participating node.
cfg.dofs_per_node        = 6;
...
% Constrained directions for every node in fixed_nodes.csv.
% For a fully clamped/ENCASTRE 6-DOF shell or beam boundary, use 1:6.
% IMPORTANT: change this to match the ACTUAL Abaqus boundary condition.
cfg.fixed_directions     = [1, 2, 3, 4, 5, 6];

% NOTE FOR THIS MODEL:
% The stiffness MTX has node IDs 5...1957 and six DOFs at every participating node.
% Model nodes that do not appear in the MTX will be filtered automatically.
...
fprintf('      MTX local DOFs detected: %s\n', mat2str(stiffness_info.detected_local_dofs));
fprintf('      MTX participating nodes: %d (ID range %d...%d)\n', ...
    stiffness_info.node_count, stiffness_info.min_node_id, stiffness_info.max_node_id);
...
if isempty(cfg.known_force_file)
    ...
    if size(known_force_data, 2) < 1 + cfg.dofs_per_node
        error(['Known-force file must contain Node_ID plus %d local load ' ...
               'components because cfg.dofs_per_node = %d.'], ...
               cfg.dofs_per_node, cfg.dofs_per_node);
    end
...
% Duplicate entries are automatically accumulated by sparse().
    ...
    detected_local_dofs = unique([dof_i; dof_j]).';
    detected_max_dof = max(detected_local_dofs);

    if any(dof_i < 1 | dof_j < 1 | ...
           dof_i ~= floor(dof_i) | dof_j ~= floor(dof_j))
        error('MTX local DOF indices must be positive integers.');
    end

    if detected_max_dof > dofs_per_node
        error([ ...
            'MTX contains local DOFs %s, but cfg.dofs_per_node = %d. ' ...
            'Set cfg.dofs_per_node to at least %d. For the uploaded matrix, ' ...
            'use cfg.dofs_per_node = 6.'], ...
            mat2str(detected_local_dofs), dofs_per_node, detected_max_dof);
    end
    ...
    participating_nodes = unique([node_i; node_j]);

    info = struct();
    info.storage_mode = storage_mode;
    info.symmetry_error = symmetry_error;
    info.detected_local_dofs = detected_local_dofs;
    info.node_count = numel(participating_nodes);
    info.min_node_id = min(participating_nodes);
    info.max_node_id = max(participating_nodes);
end
