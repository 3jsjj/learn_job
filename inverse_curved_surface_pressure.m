...
measured_global_dofs = node_dof_numbers( ...
    id_measured_raw, num_dofs_per_node);
...
virtual_global_dofs = node_dof_numbers( ...
    id_virtual, num_dofs_per_node);

valid_virtual_active = all( ...
    ismember(virtual_global_dofs, active_dofs), 2);

if exclude_measured_nodes_from_virtual
    valid_virtual_active = valid_virtual_active & ...
        ~ismember(id_virtual, id_measured);
end

if any(~valid_virtual_active)
    warning('%d 个虚拟节点被固定、无刚度或与测点重复，已剔除。', ...
...
function dof_matrix = node_dof_numbers(node_id_list, dofs_per_node)
% 将节点 ID 转换为绝对自由度编号。
%
% 例如三维实体：
% Node 5 -> [13, 14, 15]
