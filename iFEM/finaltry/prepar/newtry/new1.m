...
%%
test_inp_node_id = 103;
test_dof_component = 3;   % 测试全局 U3
test_force = 1.0;         % 1 N

%% 单节点单位力正算测试：只验证 K、边界和自由度映射

test_inp_node_id = 103;
test_dof_component = 3;
test_force = 1.0;

% INP NodeID -> node_coords 行号
[test_node_found, test_node_row] = ...
    ismember(test_inp_node_id, node_ids);
...
% INP 节点行号 -> MTX 节点编号
test_mtx_node_id = ...
    node_row_to_mtx_id(test_node_row);

if ~isfinite(test_mtx_node_id)
    error('测试节点没有 MTX 映射。');
end

% MTX 中的绝对自由度编号
test_absolute_dof = ...
    num_dofs_per_node * (test_mtx_node_id - 1) + ...
    test_dof_component;

% 转换为约束处理后的 K_global 局部自由度
[test_dof_is_active, test_local_dof] = ...
    ismember(test_absolute_dof, active_dofs);

if ~test_dof_is_active
    error('测试自由度属于固定约束或不存在。');
end

% 单节点单位力
F_point_test = sparse(size(K_global,1),1);
F_point_test(test_local_dof) = test_force;

% MATLAB 位移
U_point_test_global = ...
    K_global \ F_point_test;

fprintf('\n');
fprintf('========== 单节点单位力测试 ==========\n');
fprintf('INP NodeID：%d\n', test_inp_node_id);
fprintf('MTX NodeID：%d\n', test_mtx_node_id);
fprintf('加载方向：U%d\n', test_dof_component);
fprintf('载荷：%.6e N\n', test_force);
fprintf('MATLAB 位移范数：%.6e\n', ...
    norm(U_point_test_global));
fprintf('======================================\n');
...
%% Abaqus 与 MATLAB 全表面位移逐节点比对

abaqus_compare_data = readmatrix( ...
    'Abaqus_All_Surface_Displacement.csv');

valid_abaqus_rows = ...
    size(abaqus_compare_data,2) >= 4 & ...
    isfinite(abaqus_compare_data(:,1)) & ...
    all(isfinite(abaqus_compare_data(:,2:4)),2);

abaqus_compare_data = ...
    abaqus_compare_data(valid_abaqus_rows,:);

abaqus_node_ids = ...
    round(abaqus_compare_data(:,1));

U_abaqus_all = ...
    abaqus_compare_data(:,2:4);

% 按 NodeID 匹配
[node_is_matched, abaqus_location] = ...
    ismember(surface_node_ids, abaqus_node_ids);
...
compare_node_ids = ...
    surface_node_ids(valid_compare);

U_matlab_compare = ...
    U_surface_3D(valid_compare,:);

U_abaqus_compare = ...
    U_abaqus_all( ...
        abaqus_location(valid_compare),:);

U_error_compare = ...
    U_matlab_compare - U_abaqus_compare;

U_error_magnitude = ...
    vecnorm(U_error_compare,2,2);

U_abaqus_magnitude = ...
    vecnorm(U_abaqus_compare,2,2);

U_matlab_magnitude = ...
    vecnorm(U_matlab_compare,2,2);

fprintf('\n');
fprintf('========== Abaqus-MATLAB 节点位移对比 ==========\n');
fprintf('匹配节点数量：%d / %d\n', ...
    nnz(valid_compare), numel(surface_node_ids));

component_names_compare = {'U1','U2','U3'};

relative_error_component = nan(1,3);
correlation_component = nan(1,3);

for component_index = 1:3

    relative_error_component(component_index) = ...
        norm(U_error_compare(:,component_index)) / ...
        max(norm(U_abaqus_compare(:,component_index)),eps);

    current_corr = corrcoef( ...
        U_matlab_compare(:,component_index), ...
        U_abaqus_compare(:,component_index));

    if size(current_corr,1) == 2
        correlation_component(component_index) = ...
            current_corr(1,2);
    end

    fprintf('%s 相对 L2 误差：%.2f %%\n', ...
        component_names_compare{component_index}, ...
        100 * relative_error_component(component_index));
...
overall_relative_error = ...
    norm(U_error_compare,'fro') / ...
    max(norm(U_abaqus_compare,'fro'),eps);

overall_cosine_similarity = ...
    dot(U_matlab_compare(:),U_abaqus_compare(:)) / ...
    max(norm(U_matlab_compare(:)) * ...
        norm(U_abaqus_compare(:)),eps);

% 判断是否主要是固定比例问题
best_scale_matlab_to_abaqus = ...
    dot(U_matlab_compare(:),U_abaqus_compare(:)) / ...
    max(dot(U_matlab_compare(:),U_matlab_compare(:)),eps);

scaled_error = ...
    best_scale_matlab_to_abaqus * U_matlab_compare - ...
    U_abaqus_compare;

scaled_relative_error = ...
    norm(scaled_error,'fro') / ...
    max(norm(U_abaqus_compare,'fro'),eps);

fprintf('整体相对 L2 误差：%.2f %%\n', ...
    100 * overall_relative_error);

fprintf('整体余弦相似度：%.6f\n', ...
    overall_cosine_similarity);

fprintf('MATLAB 到 Abaqus 最佳比例：%.6e\n', ...
    best_scale_matlab_to_abaqus);

fprintf('消除固定比例后的误差：%.2f %%\n', ...
    100 * scaled_relative_error);

fprintf('最大节点误差：%.6e\n', ...
    max(U_error_magnitude));
...
comparison_table = table( ...
    ...
    'VariableNames', { ...
        'NodeID', ...
        'U1_Abaqus','U1_MATLAB','U1_Error', ...
        'U2_Abaqus','U2_MATLAB','U2_Error', ...
        'U3_Abaqus','U3_MATLAB','U3_Error', ...
        'Magnitude_Abaqus', ...
        'Magnitude_MATLAB', ...
        'Magnitude_Error'});
...
writetable( ...
    comparison_table, ...
    'Abaqus_vs_MATLAB_Full_Surface.csv');

% 输出误差最大的10个节点
[~, error_order] = sort( ...
    U_error_magnitude,'descend');

top_count = min(10,numel(error_order));

fprintf('\n误差最大的节点：\n');
disp(comparison_table( ...
    error_order(1:top_count), ...
    {'NodeID','Magnitude_Abaqus', ...
     'Magnitude_MATLAB','Magnitude_Error'}));

%% 三角形单元一致压力载荷测试

p_test = 1.0;  % MPa = N/mm^2

F_triangle_pressure = ...
    sparse(size(K_global,1),1);

for triangle_index = ...
        1:size(target_surface_triangles,1)

    triangle_node_rows = ...
        target_surface_triangles(triangle_index,:);

    x1 = node_coords(triangle_node_rows(1),:);
    x2 = node_coords(triangle_node_rows(2),:);
    x3 = node_coords(triangle_node_rows(3),:);

    % area_normal 的模长就是三角形面积
    area_normal = ...
        0.5 * cross(x2 - x1, x3 - x1);

    % 每个三角形节点分得 1/3 单元压力合力
    nodal_force_vector = ...
        pressure_sign * p_test * area_normal / 3;

    for local_node_index = 1:3

        current_node_row = ...
            triangle_node_rows(local_node_index);

        current_mtx_node_id = ...
            node_row_to_mtx_id(current_node_row);

        if ~isfinite(current_mtx_node_id)
            continue;
        end

        current_absolute_dofs = ...
            node_dof_numbers( ...
                current_mtx_node_id, ...
                num_dofs_per_node, ...
                translation_dofs);

        [all_active, current_local_dofs] = ...
            ismember(current_absolute_dofs,active_dofs);

        if all(all_active)
            F_triangle_pressure(current_local_dofs) = ...
                F_triangle_pressure(current_local_dofs) + ...
                nodal_force_vector(:);
        end
    end
...
U_triangle_pressure = ...
    K_global \ F_triangle_pressure;

fprintf('\n');
fprintf('三角形一致压力总力：\n');
%% 按 active_dofs 对应的真实自由度分量统计合力

active_dof_components = ...
    mod(active_dofs - 1, num_dofs_per_node) + 1;

is_force_x = active_dof_components == 1;
is_force_y = active_dof_components == 2;
is_force_z = active_dof_components == 3;

total_force_x = full(sum( ...
    F_triangle_pressure(is_force_x)));

total_force_y = full(sum( ...
    F_triangle_pressure(is_force_y)));

total_force_z = full(sum( ...
    F_triangle_pressure(is_force_z)));

total_force_magnitude = norm([ ...
    total_force_x, ...
    total_force_y, ...
    total_force_z]);
...
fprintf('总等效力 Fx：%.6e N\n', ...
    total_force_x);

fprintf('总等效力 Fy：%.6e N\n', ...
    total_force_y);

fprintf('总等效力 Fz：%.6e N\n', ...
    total_force_z);

fprintf('总等效力模长：%.6e N\n', ...
    total_force_magnitude);

fprintf('三角形压力位移范数：%.6e\n', ...
    full(norm(U_triangle_pressure)));
